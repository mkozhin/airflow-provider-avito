...
- [x] внешнее поведение не изменилось: возвраты, тексты ошибок, формат JSON/CSV
- [x] секреты не присутствуют в результате `get_accounts`
- [x] запустить полный `pytest`

### Task 5: [Final] Завершение
- [x] обновить `CONTEXT.md`, если по ходу всплыли новые понятия
