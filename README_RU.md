...
Каждая запись содержит 17 полей. Канонический упорядоченный список имён полей также доступен как `CALL_FIELDS` (константа `tuple[str, ...]`, экспортируемая из `airflow_provider_avito.hooks.avito`) — для использования в определениях схем или валидации на стороне потребителя.
